            ...
            For f_rw = 2 To num_LastRow
                ...
                If Len(nm_mreg_EXT) > 0 Then
                    ...
                    For f_p = 1 To 2
                        sts_add2dic = False
                        Select Case f_p
                            Case 1: keyUser = f_year & nm_month & nm_FLSM: sts_add2dic = IIf(InStr(1, LCase(nm_FLSM), "вакан", vbTextCompare) <> 0, False, True): Person = nm_FLSM
                            Case 2: keyUser = f_year & nm_month & nm_Srep: If nm_vacancy_status = "active" Then sts_add2dic = True: Person = nm_Srep
                        End Select

                        If Not dic_People.Exists(keyUser) And sts_add2dic = True And Len(Person) > 0 Then
    ...
    For Each smu In smr
        With smu
            ...
            If Not dic_UserEducated.Exists(keyEduUSR) And year(.EduDate) = f_year Then
                ...
                objEduUSR.EducationType = .EducationType
...
For Each UserEducated In dic_UserEducated.Items
    ...
    With UserEducated
        ...
        n = n + 1: Cells(i, n) = IIf(i = 1, "EducationType", .EducationType)
